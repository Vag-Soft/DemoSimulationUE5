...
'''
Learnt nothing i think
'''
class Env(gym.Env):
    def __init__(self, width, height, cam_count, max_skeletons, fake_client=None, mode="train"):
        super().__init__()
        ...
        # self.observation_space = spaces.Box(low=0, high=1, shape=(cam_count, 2048), dtype=np.float32)
        self.observation_space = spaces.Dict({
            "features": spaces.Box(low=0, high=1, shape=(cam_count, 2048), dtype=np.float32),
            "previous_binary_mask": spaces.MultiBinary(cam_count),
            "previous_poses": spaces.Box(low=-np.inf, high=np.inf, shape=(cam_count, max_skeletons, 13, 2), dtype=np.float32),
            "previous_confidences": spaces.Box(low=0, high=1, shape=(cam_count, max_skeletons, 13), dtype=np.float32),
            "camera_rotation_matrices": spaces.Box(low=-np.inf, high=np.inf, shape=(cam_count, 3, 3), dtype=np.float32),
            "camera_locations": spaces.Box(low=-np.inf, high=np.inf, shape=(cam_count, 3), dtype=np.float32),
            "camera_intrinsics": spaces.Box(low=-np.inf, high=np.inf, shape=(cam_count, 5), dtype=np.float32), # fov_rad, fx, fy, cx, cy
        })
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.current_obs["features"] = features

        self.current_obs["previous_binary_mask"] = np.zeros((self.cam_count,), dtype=np.int8)

        rotation_matrices = np.array([transform_utils.euler_to_rotation_matrix(self.client.camera_rotations[i]) for i in range(self.cam_count)])
        self.current_obs["camera_rotation_matrices"] = rotation_matrices

        self.current_obs["camera_locations"] = np.array(self.client.camera_locations)

        camera_intrinsics = np.array([transform_utils.calculate_intrinsics(self.camera_fov_deg, self.height, self.width) for _ in range(self.cam_count)])
        self.current_obs["camera_intrinsics"] = camera_intrinsics

        self.current_obs["previous_poses"] = np.zeros((self.cam_count, self.max_skeletons, 13, 2), dtype=np.float32)
        self.current_obs["previous_confidences"] = np.zeros((self.cam_count, self.max_skeletons, 13), dtype=np.float32)
    ...
    def step(self, action):
        ...
        self.current_obs["previous_binary_mask"] = np.zeros((self.cam_count,), dtype=np.int8)
        self.current_obs["previous_binary_mask"][self.current_camera] = 1

        self.current_obs["previous_poses"] = np.zeros((self.cam_count, self.max_skeletons, 13, 2), dtype=np.float32)
        if processed_skeletons is not None:
            self.current_obs["previous_poses"][self.current_camera] = processed_skeletons[:self.max_skeletons]

        self.current_obs["previous_confidences"] = np.zeros((self.cam_count, self.max_skeletons, 13), dtype=np.float32)
        if confidences is not None:
            self.current_obs["previous_confidences"][self.current_camera] = confidences[:self.max_skeletons]
