...
class Env(gym.Env):
    def __init__(self, width, height, cam_count, max_skeletons, history=5, fake_client=None, mode="train"):
        ...
        self.observation_space = spaces.Dict({
            ...
            "previous_poses": spaces.Box(low=-np.inf, high=np.inf, shape=(history, max_skeletons, 13, 2), dtype=np.float32),
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.current_obs["previous_poses"] = np.zeros((self.history, self.max_skeletons, 13, 2), dtype=np.float32)
