...
from PythonClient.utils import plot_utils
from PythonClient.utils.transform_utils import project_3D_to_2D
...
class Env(gym.Env):
    def __init__(self, width, height, cam_count, max_skeletons, fake_client=None, mode="train"):
        ...
        self.current_obs = np.zeros((cam_count, 2048), dtype=np.float32)
    ...
    def calculate_reward(self):
        pose_results = self.pose_estimator.estimate_pose(self.current_images[self.current_camera])
        if pose_results is None:
            return -1

        processed_skeletons, _c = self.pose_estimator.process_results(pose_results)

        projected_skeletons = project_3D_to_2D(
