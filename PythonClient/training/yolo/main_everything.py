...
from PythonClient.envs.yolo.env_everything_v5 import Env
...
if __name__ == "__main__":
    ...
    base_model_path = "E:/Vag/Programs/UnrealEngine/UE 5.5/DemoSimulationUE5/PythonClient/models/yolo/EverythingV5_1P_3C_5S_A2C_500000"
    ...
    while True:
        ...
        steps += 2000
        model_path = os.path.join(base_model_path, "A2C_" + str(500000) + "_steps.zip")
    ...
    print(f"Best model with sum {np.max(sums)} at step {sums.index(np.max(sums)) * 5000}")
    print(f"Best model with avg {np.max(avgs)} at step {avgs.index(np.max(avgs)) * 5000}")
    print(f"Best model with unsuccessful {np.min(unsuccessful)} at step {unsuccessful.index(np.min(unsuccessful)) * 5000}")