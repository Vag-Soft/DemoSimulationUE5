...
from my_romp.post_parser import parsing_outputs
...
from my_romp.utils import img_preprocess, create_OneEuroFilter, check_filter_state, \
...
def bev_settings(input_args=sys.argv[1:]):
    ...
    parser.add_argument('--model_id', type=int, default=2,
                        help='Whether to process the input as a long image, sliding window way')
    parser.add_argument('-i', '--input', type=str, default='/home/yusun/CenterMesh/simple_romp/test/ages.png',
                        help='Path to the input image / video')
    parser.add_argument('-o', '--save_path', type=str, default=osp.join(osp.expanduser("~"), 'BEV_results'),
                        help='Path to save the results')
    parser.add_argument('--crowd', action='store_false',
                        help='Whether to process the input as a long image, sliding window way')
    ...
    parser.add_argument('--overlap_ratio', type=float, default=long_conf_dict[model_id][3],
                        help='The frame_rate of saved video results')
    parser.add_argument('--center_thresh', type=float, default=conf_dict[model_id][0],
                        help='The confidence threshold of positive detection in 2D human body center heatmap.')
    parser.add_argument('--nms_thresh', type=float, default=conf_dict[model_id][1],
                        help='The 2D-pose-projection similarity threshold of suppressing overlapping predictions.')
    parser.add_argument('--relative_scale_thresh', type=float, default=conf_dict[model_id][2],
                        help='The confidence threshold of positive detection in 2D human body center heatmap.')
    parser.add_argument('--show_largest', action='store_true', help='Whether to show the largest person only')
    parser.add_argument('--show_patch_results', action='store_true',
                        help='During processing long image, whether to show the results of intermediate results of each patch.')
    ...
    parser.add_argument('--renderer', type=str, default='sim3dr',
                        help='Choose the renderer for visualizaiton: pyrender (great but slow), sim3dr (fine but fast), open3d (webcam)')
    parser.add_argument('--render_mesh', action='store_false',
    ...
    parser.add_argument('--show_items', type=str, default='mesh,mesh_bird_view',
                        help='The items to visualized, including mesh,pj2d,j3d,mesh_bird_view,mesh_side_view,center_conf,rotate_mesh. splited with ,')
    ...
    parser.add_argument('--smpl_path', type=str, default=osp.join(osp.expanduser("~"), '.romp', 'SMPLA_NEUTRAL.pth'),
                        help='The path of SMPL-A model file')
    parser.add_argument('--smil_path', type=str, default=osp.join(osp.expanduser("~"), '.romp', 'smil_packed_info.pth'),
                        help='The path of SMIL model file')
    parser.add_argument('--model_path', type=str, default=osp.join(osp.expanduser("~"), '.romp', model_dict[model_id]),
                        help='The path of BEV checkpoint')
    parser.add_argument('--model_onnx_path', type=str, default=osp.join(osp.expanduser("~"), '.romp', 'BEV.onnx'),
                        help='The path of BEV onnx checkpoint')
...
class BEV(nn.Module):
    ...
    def _initilization_(self):
        ...
        self.result_keys = ['smpl_thetas', 'smpl_betas', 'cam', 'cam_trans', 'params_pred', 'center_confs',
                            'pred_batch_ids']
    ...
    def _initialize_optimization_tools_(self, smooth_coeff):
        ...
        if not self.settings.show_largest:
            ...
            self.tracker = Tracker(det_thresh=0.12, low_conf_det_thresh=0.05, track_buffer=60, match_thresh=300,
                                   frame_rate=30)
    ...
    def single_image_forward(self, image):
        if self.settings.onnx:
            ...
            input_numpy = input_image.numpy().astype(np.float32)
            print('input_numpy shape:', input_numpy.shape)

            # Run inference with ONNX model
            # Adjust input name based on your exported model
            ort_inputs = {'image': input_numpy}
            ort_outputs = self.ort_session.run(None, ort_inputs)
            print(ort_outputs)

            # Convert ONNX outputs to the expected format
            # This mapping depends on how you exported your model
            parsed_results = {}
            output_names = ['params_pred', 'cam_czyx', 'center_map', 'center_map_3d', 'pred_batch_ids', 'pred_czyxs',
                            'center_confs']  # Adjust based on your model outputs
            for i, name in enumerate(output_names):
                if i < len(ort_outputs):
                    parsed_results[name] = torch.from_numpy(ort_outputs[i])
        ...
        #     center_maps, params_maps = torch.from_numpy(center_maps).to(self.tdevice), torch.from_numpy(params_maps).to(
        #         self.tdevice)
        # else:
    ...
    def forward(self, image, signal_ID=0, **kwargs):
        ...
        if self.settings.render_mesh:
            ...
            rendering_cfgs = {'mesh_color': mesh_color_type, 'items': self.visualize_items,
        ...
        if self.settings.show:
            ...
            show_image = outputs['rendered_image'] if h <= 1080 else cv2.resize(outputs['rendered_image'],
                                                                                (int(w * (1080 / h)), 1080))
    ...
    def process_long_image(self, full_image, show_patch_results=False):
        ...
        full_image_pad, image_pad_info, pad_length = padding_image_overlap(full_image,
        ...
        for cid in range(len(crop_boxes)):
            ...
            if this_outs is not None:
                ...
                projection = body_mesh_projection2image(this_outs['joints'], this_outs['cam'],
                                                        vertices=this_outs['verts'],
                                                        input2org_offsets=torch.Tensor([0, ch, 0, cw, ch, cw]))
        ...
        for cid, crop_image in enumerate(croped_images):
            this_outs = outputs_list[cid]
            ch, cw = croped_images[cid].shape[:2]
            this_outs = suppressing_redundant_prediction_via_projection(this_outs, [ch, cw],
                                                                        thresh=self.settings.nms_thresh,
                                                                        conf_based=True)
            this_outs = remove_outlier(this_outs, scale_thresh=1,
                                       relative_scale_thresh=self.settings.relative_scale_thresh)

        if show_patch_results:
            rendering_cfgs = {'mesh_color': 'identity', 'items': ['mesh', 'center_conf', 'pj2d'],
                              'renderer': self.settings.renderer}
            for cid, crop_image in enumerate(croped_images):
                this_outs = outputs_list[cid]
        ...
        for cid, crop_box in enumerate(crop_boxes):
            ...
            crop_outputs['cam'] = convert_crop_cam_params2full_image(crop_outputs['cam'], crop_box,
                                                                     full_image.shape[:2])
        ...
        projection = body_mesh_projection2image(outputs['joints'], outputs['cam'], **meta_data)
        outputs.update(projection)
        outputs = suppressing_redundant_prediction_via_projection(outputs, full_image.shape,
                                                                  thresh=self.settings.nms_thresh, conf_based=True)
    ...
    def temporal_optimization(self, outputs, signal_ID, image_scale=128, depth_scale=30):
        ...
        if self.settings.show_largest:
            ...
            tracking_points = np.concatenate(
                [(cams[:, [2, 1]] + 1) * image_scale, cam_trans[:, [2]] * depth_scale, cams[:, [0]] * image_scale / 2],
                1)
